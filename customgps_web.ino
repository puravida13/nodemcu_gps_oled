...
void loop() {
  ...
  display.display();                                           //Display buffer to screen
  delay(4000);                                                 //4 second pause
  compass_disp();                                              //call compass display function
  display.clearDisplay();                                      //Clear display
  display.setCursor(0,0);                                      //Set cursor to upper left
...
void compass_disp(){
  ...
  display.setCursor(0,0);                                      //Set cursor to upper left
  display.setTextSize(2);                                  // Normal 1:1 pixel scale
  display.println("Direction:");                               //Display text

  if ((gps.course.deg()) >= 0 && (gps.course.deg()) <= 10 ){
    display.setCursor(40,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("N");
    }
  
  if ((gps.course.deg()) >= 350 && (gps.course.deg()) <= 359 ){
    display.setCursor(50,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("N");
   }
   if ((gps.course.deg()) >= 11 && (gps.course.deg()) <= 80 ){
    display.setCursor(40,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("NE");
   }
   if ((gps.course.deg()) >= 81 && (gps.course.deg()) <= 100 ){
    display.setCursor(50,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("E");
   }
   if ((gps.course.deg()) >= 101 && (gps.course.deg()) <= 170 ){
    display.setCursor(40,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("SE");
   }
   if ((gps.course.deg()) >= 171 && (gps.course.deg()) <= 190 ){
    display.setCursor(50,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("S");
   }
   if ((gps.course.deg()) >= 191 && (gps.course.deg()) <= 260 ){
    display.setCursor(40,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("SW");
   }
   if ((gps.course.deg()) >= 261 && (gps.course.deg()) <= 280 ){
    display.setCursor(50,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("W");
   }
   if ((gps.course.deg()) >= 281 && (gps.course.deg()) <= 349 ){
    display.setCursor(40,30);
    display.setTextSize(4);                                  // Normal 1:1 pixel scale
    display.println("NW");
   }
